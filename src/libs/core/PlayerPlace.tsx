...
const useStyles = createUseStyles<
  string,
  { isMe: boolean; isPlayerTurn: boolean },
  any
>((theme: Theme) => ({
  container: (props) => ({
    ...theme.basicFlex,
  ...
  title: (props) => ({
    ...
    boxShadow: props.isPlayerTurn
      ? `0px 0px 30px 3px ${theme.colors.orange}`
      : undefined,
  }),
  text: {
    ...theme.fonts.h5,
...
export const PlayerPlace = ({ isMe = false, name }: Props) => {
  ...
  const classes = useStyles({
    theme,
    isMe,
    isPlayerTurn: !!(board && player?.id === board.currentPlayerId),
  });
    ...
    <>
          ...
          {!isMe && isMyTurn && (
            <>
              <Button
                action={true}
                square={true}
                classNameUpdate={classes.button}
                onClick={() => setModalRobber(true)}
                disabled={!isPossibleAction(ActionType.ROB_COIN)}>
                <Icons icon={Icon.robber} size={theme.icon.normal + 10} />
              </Button>
              <Button
                action={true}
                square={true}
                classNameUpdate={classes.button}
                onClick={() => setModalKill(true)}
                disabled={
                  !isPossibleAction(ActionType.KILL_COIN_7) &&
                  !isPossibleAction(ActionType.KILL_COIN_3)
                }>
                <Icons icon={Icon.knife} size={theme.icon.normal + 10} />
              </Button>
            </>
          )}
          {isMe && (
            <Button
              ...
              disabled={
                !isPossibleAction(ActionType.BLOCK_PICK) &&
                !isPossibleAction(ActionType.BLOCK_ROB) &&
                !isPossibleAction(ActionType.BLOCK_KILL)
              }>
              <Icons icon={Icon.block} size={theme.icon.normal + 10} />
            </Button>
          )}
        </div>
        {player && (
          <div className={classes.containerCard}>
            <Card
              pin
              hide={!isMe}
              cardType={player.cards[0].power}
              isDestroyed={player.cards[0].isDestroyed}
            ...
            <Card
              pin
              hide={!isMe}
              cardType={player.cards[1].power}
              isDestroyed={player.cards[1].isDestroyed}
              disabled={!isPossibleAction(ActionType.CHOOSE_CARD)}
              onClick={async () => {
                if (!myPLayer) return;
                await action({
      ...
      <Modal isOpen={modalBlock} setIsOpen={setModalBlock} title="Blockage">
        <div className={classes.modalContainer}>
          <Button
            full
            warning={isWarnAction(ActionType.BLOCK_KILL)}
          ...
          <Button
            full
            warning={isWarnAction(ActionType.BLOCK_ROB)}
          ...
          <Button
            full
            warning={isWarnAction(ActionType.BLOCK_PICK)}

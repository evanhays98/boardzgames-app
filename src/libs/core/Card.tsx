...
const useStyles = createUseStyles<string, { pin: boolean, hide: boolean }, any>((theme: Theme) => ({
  container: props => ({
...
export const Card = ({ pin, hide = false, cardType, className, style, isDestroyed, onClick, disabled }: Props) => {
          ...
          <>
            ...
            {cardType === CardType.DUCHESS &&
              <>
                ...
                <h1 className={classes.title}>Duchess</h1>
              </>
            }
            {cardType === CardType.COUNTESS &&
              <>
                <img src={comtesse} alt='comtesse' className={classes.img} />
                <h1 className={classes.title}>Comtesse</h1>
              </>
            }
            {cardType === CardType.AMBASSADOR &&
              <>
                <img src={ambassador} alt='ambassador' className={classes.img} />
                <h1 className={classnames(classes.title)}>Ambassador</h1>
              </>
            }
            {cardType === CardType.ASSASSIN &&
              <>
                <img src={murderer} alt='MURDERER' className={classes.img} />
                <h1 className={classnames(classes.title)}>Assassin</h1>
              </>
            }
            {cardType === CardType.CAPTAIN &&
              <>
                <img src={captain} alt='CAPTAIN' className={classes.img} />
                <h1 className={classnames(classes.title)}>Captain</h1>
              </>
            }
            {isDestroyed &&
              <>
